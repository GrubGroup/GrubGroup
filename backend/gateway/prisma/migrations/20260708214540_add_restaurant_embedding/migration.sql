...
-- CreateIndex (manual: HNSW ANN index for cosine similarity search on embeddings.
-- Not expressible in the Prisma schema; keep in sync if the distance metric changes.)
CREATE INDEX "Restaurant_embedding_hnsw" ON "Restaurant" USING hnsw ("embedding" vector_cosine_ops);
