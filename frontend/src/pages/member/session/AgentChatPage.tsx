...
// Quick-reply suggestion chips shown under the latest agent message.
const QUICK_REPLIES = ['Near the office', "I'll type a place", 'Anywhere works', '$15–20pp']
...
export function AgentChatPage() {
  ...
  }, [members.length, loadSession, currentUserId, messages.length, seed, phase, setPhase])
            ...
            <div className="flex flex-wrap gap-2 px-5 pb-1 pt-2">
              {QUICK_REPLIES.map((q) => (
                <button
                  ...
                  className="rounded-pill border border-border-strong bg-surface-raised px-3 py-1.5 text-xs font-medium text-text hover:bg-surface-sunken"
                >
                  {q}
                </button>
              ))}
            </div>
            <VoiceComposer onSend={sendUserMessage} privacyNote />
            <button
              onClick={handleDone}
              className="flex items-center justify-center gap-1 border-t border-border bg-surface-raised py-2 text-center text-xs font-medium text-primary hover:text-primary-hover"
            >
